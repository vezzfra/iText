...
class Text5: WKInterfaceController, WCSessionDelegate {
    ...
    override func awakeWithContext(context: AnyObject?) {
        ...
        if (NSUserDefaults.standardUserDefaults().objectForKey("Text5_Text") == nil) {
            
            text.setText("Add a text on your iPhone's App")
            
        } else {
            
            let string1 = NSUserDefaults.standardUserDefaults().objectForKey("Text5_Text") as! String
    ...
    func session(session: WCSession, didReceiveMessage message: [String : AnyObject]) {
        ...
        self.text.setText(message["m4"]! as? String)
        
        NSUserDefaults.standardUserDefaults().setObject(message["m4"], forKey: "Text5_Text")
    ...
    @IBAction func sync(sender: WKInterfaceButton) {
        ...
        let action1 = WKAlertAction(title: "OK", style: .Default) {
            ...
            if (!(NSUserDefaults.standardUserDefaults().objectForKey("Text5_Text") == nil)) {
                self.session.sendMessage(["m5":NSUserDefaults.standardUserDefaults().objectForKey("Text5_Text") as! String], replyHandler: {replyMessage in
