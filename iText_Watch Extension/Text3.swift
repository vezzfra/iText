...
class Text3: WKInterfaceController, WCSessionDelegate {
    ...
    override func awakeWithContext(context: AnyObject?) {
        ...
        if (NSUserDefaults.standardUserDefaults().objectForKey("Text3_Text") == nil) {
            
            text.setText("Add a text on your iPhone's App")
            
        } else {
            
            let string1 = NSUserDefaults.standardUserDefaults().objectForKey("Text3_Text") as! String
    ...
    func session(session: WCSession, didReceiveMessage message: [String : AnyObject]) {
        ...
        self.text.setText(message["m2"]! as? String)
        
        NSUserDefaults.standardUserDefaults().setObject(message["m2"], forKey: "Text3_Text")
    ...
    @IBAction func sync(sender: WKInterfaceButton) {
        ...
        let action1 = WKAlertAction(title: "OK", style: .Default) {
            ...
            if (!(NSUserDefaults.standardUserDefaults().objectForKey("Text3_Text") == nil)) {
                self.session.sendMessage(["m7":NSUserDefaults.standardUserDefaults().objectForKey("Text3_Text") as! String], replyHandler: {replyMessage in
